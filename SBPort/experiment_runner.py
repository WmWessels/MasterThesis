...
from utils import prepare_openml_for_inf, sklearn_to_gama_str, extend_pipeline
        ...
        for dataset_id in self._dataset_ids:
            ...
            if self._task == "regr":
                clf = GamaRegressor(
                    ...
                    scoring = self.scoring,
                    n_jobs = -1,
                    output_directory = Path(__file__).parent / f"gama_logs_{dataset_id}_{max_total_time/60}_{warm_start_postfix}"
                    )
            else:
                clf = GamaClassifier(
                    max_total_time = max_total_time, 
        ...
        for dataset_id in self._dataset_ids:
            ...
            for label in range(n_clusters):
                ...
                if label == predicted_label:
                    results[dataset_id]["predicted"] = result
                else:
                    results[dataset_id][label] = result
...
def main():
    ...
    argparser.add_argument("--experiment", type = str, choices = ["grid_search", "one_nn", "cluster_vs_all", "static", "optimal", "askl2", "automl_10", "automl_60", "automl_ws_10", "automl_ws_60"])
    ...
    experiment_runner = ExperimentRunner(task, id_mapper[str(task)]["test"][5:], sbport, **kwargs)
    # experiment_runner = ExperimentRunner(task, [23517], sbport, **kwargs)
    ...
    elif args.experiment == "optimal":
        experiment_runner.run_optimal_configuration(path_to_optimal_dir)
